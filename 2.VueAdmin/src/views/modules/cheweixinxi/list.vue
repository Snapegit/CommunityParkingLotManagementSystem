		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0 0 20px","borderRadius":"8px","flexWrap":"wrap","background":"none","display":"flex","width":"100%"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"auto","margin":"20px 20px 20px 0","display":"inline-block"}' >
					...
					<div :style='{"margin":"0 5px 0 0","display":"inline-block"}' class="select" label="区域" prop="quyu">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="cheweitupian" width="200" label="车位图片">
						<template slot-scope="scope">
							<div v-if="scope.row.cheweitupian">
								<img v-if="scope.row.cheweitupian.substring(0,4)=='http'" :src="scope.row.cheweitupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.cheweitupian.split(',')[0])">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('cheweixinxi','停车')" @click="chezijinchangCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
...
import chezijinchangCrossAddOrUpdate from "../chezijinchang/add-or-update";
	export default {
		...
		methods: {
			...
			chezijinchangCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
