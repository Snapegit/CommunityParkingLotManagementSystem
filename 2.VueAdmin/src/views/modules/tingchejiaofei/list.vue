<template>
	<div class="main-content" :style='{"padding":"30px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0 0 20px","borderRadius":"8px","flexWrap":"wrap","background":"none","display":"flex","width":"100%"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"auto","margin":"20px 20px 20px 0","display":"inline-block"}' >
					...
					<div :style='{"margin":"0 5px 0 0","display":"inline-block"}' class="select">
						<label :style='{"margin":"0 5px 0 0","color":"#000","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">是否支付</label>
				...
				<el-row class="actions" :style='{"width":"auto","margin":"20px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('tingchejiaofei','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('tingchejiaofei','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						删除
					</el-button>
					...
					<el-button class="btn18" v-if="isAuth('tingchejiaofei','日营业额')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0","overflow":"hidden","borderRadius":"8px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="cheliangtupian" width="200" label="车辆图片">
						<template slot-scope="scope">
							<div v-if="scope.row.cheliangtupian">
								<img v-if="scope.row.cheliangtupian.substring(0,4)=='http'" :src="scope.row.cheliangtupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.cheliangtupian.split(',')[0])">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="bencitingcheshizhang"
						label="本次停车时长">
						<template slot-scope="scope">
							{{scope.row.bencitingcheshizhang}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="bencitingchefeiyong"
						label="停车费用">
						<template slot-scope="scope">
							{{scope.row.bencitingchefeiyong}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							<span style="margin-right:10px">{{scope.row.ispay=='已支付'?'已支付':'未支付'}}</span>
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('tingchejiaofei','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('tingchejiaofei','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('tingchejiaofei','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('tingchejiaofei','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="bencitingchefeiyongChart1" style="width:100%;height:600px;"></div>
		...
		methods: {
			...
			payHandler(row){
				let params = {
					...
					totalamount: row.bencitingchefeiyong,
				}
				this.$http({
					url: "tingchejiaofei/alipay",
					method: "get",
					params: params
				}).then(({ data }) => {
						if (data && data.code === 0) {
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var bencitingchefeiyongChart1 = echarts.init(document.getElementById("bencitingchefeiyongChart1"),'macarons');
        this.$http({
            url: `tingchejiaofei/value/lichangshijian/bencitingchefeiyong/日`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].lichangshijian);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].lichangshijian
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                bencitingchefeiyongChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    bencitingchefeiyongChart1.resize();
                };
            }
        });
      })
	...
	tingchejiaofeistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'tingchejiaofei/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
